...
export class Carousel extends Component {
  ...
  showSlides = (fromButtons = false, dir = 1) => {
    ...
    const index = (this.state.slideIndex + dir) % ( this.state.slides.length + 1) || dir < 0 &&  this.state.slides.length || 1;
    ...
    this.setState({
      ...
      () => {
        ...
        !fromButtons && setTimeout(this.showSlides.bind(this), 5000);
      }
    );
  };
    ...
    return (
      <div className="carusel">
        <div className="slideshow-container">
          ...
          <button className="previous shange-slide_btn" onClick={() => {
              this.plusSlides(-1);
            }}/>
          <button className="next shange-slide_btn" onClick={() => {
              this.plusSlides(1);
            }}/>
        </div>
        <div>
          <ul className='dots'>
            {slidesUrl.map((url,index)=>{
              return (<li className="dot" key={index+10} onClick={() => {
